function OSB_dm_test(){
  ...
  OSB_dm.download( fail_url);
  //OSB_dm.download( img_url, "my-test-photo.png",[12345]);
  OSB_dm.downloads();
 }
...
var download=function(url, save_as, extra) {
  var fsize=0;
...
var filename_=new android.File( url.split("?")[0] ).getName();

var filename = save_as||filename_; //url.replace(/^.*[\\\/]/, '');
...
function refreshDownloadsCheck(){
...
var getDownloadStatus=function(id) {
  ...
  var lastModified = cursor.getColumnIndex("last_modified");
